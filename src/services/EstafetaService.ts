...
const {mailTime,mailService,mailTo,mailFrom,mailPassword,mailOfsset} = config.sendMail;
...
export default class EstafetaService {
  ...
  async sendPosicionamiento(data: EstafetaInfo[]) {
    try{
      const urlEstafeta = `${estafetaUrl}`;
      const headers = {
        'Content-Type': 'text/xml; charset=utf-8',
      ...
      if(response.status === 200){
        console.log('Se envio correctamente el posicionamiento',response.data);
        // await this.sendEmail(response.data,'Posicionamiento');
      }
  ...
  async sendBotonPanico(data: BotonPanicoInterface[]) {
    try{
      ...
      if(response.status === 200){
        console.log('Se envio correctamente el boton de panico',response.data);
        // await this.sendEmail(response.data,'Boton de Panico');
      }
      // console.log('Boton Panico',response.data);
    }
    }catch (error) {
  ...
  //Function to send email when the data is sent to estafeta
  async sendEmail(data: any,service:string) {
    ...
    let mailOptions = {
      from: mailFrom,
      to: mailTo,
      subject: `Data de ${service}  enviada correctamente`,
      text: `Los datos ${service} son: ${JSON.stringify(data)}`
    };
  ...
  async sendVelocidadMayor80(data: VelocidaMayor80Interface[]) {
    try{
      ...
      if(response.status === 200){
        console.log('Se envio correctamente la velocidad mayor a 80',response.data);
        // await this.sendEmail(response.data,'Velocidad Mayor a 80');
      }
  ...
  async sendVelocidadMayor120(data: EstafetaInfo[]) {
    try{
      ...
      if(response.status === 200){
        console.log('Se envio correctamente la velocidad mayor a 120',response.data);
        // await this.sendEmail(response.data,'Velocidad Mayor a 120');
      }
  ...
  async sendSpeedLock(data: SpeedLockInterface[]) {
    try{
      ...
      if(response.status === 200){
        console.log('Se envio correctamente el speed lock',response.data);
        // await this.sendEmail(response.data,'Speed Lock');
      }
  ...
  async sendEntrandoGeocerca(data: EntrandoGeocercasInterface[]) {
    try{
      ...
      if(response.status === 200){
        console.log('Se envio correctamente el entrando geocerca',response.data);
        // await this.sendEmail(response.data,'Entrando Geocerca');
      }
      // console.log(response.data);
    }
    }catch (error) {
  ...
  async sendSaliendoGeocerca(data: SaliendoGeocercaInterface[]) {
    try{
      ...
      if(response.status === 200){
        console.log('Se envio correctamente el saliendo geocerca',response.data);
        // await this.sendEmail(response.data,'Saliendo Geocerca');
      }
      // console.log(response.data);
    }
    }catch (error) {

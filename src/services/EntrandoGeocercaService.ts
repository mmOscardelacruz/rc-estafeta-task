...
export default class EntrandoGeocercaReportService{
  ...
  async getData(): Promise<any[]>{
    try{
      ...
      // const events = await this.geotabService.getExceptions("2023-10-24T12:16:44.063Z","2023-10-24T15:16:44.063Z","aQShcIMK3m0Wz4N7O3_etkw")
      const events = await this.geotabService.getExceptions(`${fromDate}`,`${toDate}`,"aQShcIMK3m0Wz4N7O3_etkw")
      ...
      const result = await Bluebird.map(
        events, async event => {
          ...
          // console.log(placa,numeroSerial);
          // const placa = deviceArr.map(device => device.licensePlate)[0];
          // const serialNumber = deviceArr.map(device => device.serialNumber)[0];
          ...
          const res: EntrandoGeocercasInterface = {
            altitude: 0,
            asset: placaFormat,
            battery: 0,
            code: 'aQShcIMK3m0Wz4N7O3_etkw',
            course: 0,
            customerid: '',
            customername: '',
            date: fromDate,

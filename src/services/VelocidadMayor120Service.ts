...
export default class VelocidadMayor120ReportService{
  ...
  async getData(): Promise<any[]>{
    try{
      ...
      // const events = await this.geotabService.getExceptions("2023-10-24T18:16:44.063Z","2023-10-24T19:16:44.063Z","aMlICNRAoBEW4kxgSJGy9xA")
      const events = await this.geotabService.getExceptions(`${fromDate}`,`${toDate}`,"aMlICNRAoBEW4kxgSJGy9xA")
      ...
      const result = await Bluebird.map(
        events, async event => {
          ...
          const res: VelocidadMayor120Interface = {
            altitude: 0,
            asset: placaFormat,
            battery: 0,
            code: 'aMlICNRAoBEW4kxgSJGy9xA',
            course: 0,
            customerid: '',
            customername: '',
            date: fromDate,
